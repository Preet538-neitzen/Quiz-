...
class QB2 extends Component{
    ...
    {this.state.responses!==5?(<><div className='h1'>Math Quiz !!!</div><p>Time elapsed: {runningTime/1000}seconds</p>
    ...
    this.state.questionBank.map(({question,answers,correct,questionId})=>(
      <QuestionBox  question={question} options={answers} key={questionId} selected={answer=>this.computeAnswer(answer,correct)}/>
          ...
          {this.state.responses === 5?(<><Result score={this.state.score} playAgain={this.playAgain} baser={this.state.baser}  runningTime={runningTime} /> </>):null}
        </div>):null}
    </>
    );
}
